import React from 'react';
import { Profile } from '../../types';

interface VerificationsManagementProps {
    allProfiles: Profile[];
    onUpdateProfiles: (profiles: Profile[]) => void;
    onCreateNotification: (userId: string, message: string) => void;
}

const VerificationsManagement: React.FC<VerificationsManagementProps> = ({ allProfiles, onUpdateProfiles, onCreateNotification }) => {
    const pendingVerifications = allProfiles.filter(
        p => p.documentForVerification && !p.isVerified
    );

    const handleVerification = (userId: string, action: 'approved' | 'rejected') => {
        const updatedProfiles = allProfiles.map(profile => {
            if (profile.id === userId) {
                return {
                    ...profile,
                    isVerified: action === 'approved',
                    documentForVerification: undefined, // Clear the document after review
                };
            }
            return profile;
        });

        onUpdateProfiles(updatedProfiles);
        
        const user = allProfiles.find(p => p.id === userId);
        if (user) {
            const message = action === 'approved'
                ? 'Your identity document has been verified! Your profile now has a blue tick.'
                : 'Your identity document could not be verified. Please try uploading again or contact support.';
            onCreateNotification(userId, message);
        }
    };

    return (
        <div>
            <p className="text-sm text-theme-text-secondary mb-4">
                Review submitted identity documents to grant users a 'Verified' status (blue tick).
            </p>
            {pendingVerifications.length === 0 ? (
                <div className="text-center p-8 bg-theme-bg/50 rounded-lg border border-theme-border">
                    <p className="text-theme-text-secondary">No identity verifications are pending.</p>
                </div>
            ) : (
                <div className="overflow-x-auto">
                    <table className="w-full text-left">
                        <thead className="bg-theme-bg/50">
                            <tr className="border-b border-theme-border">
                                <th className="p-3 text-sm font-semibold text-theme-text-secondary">User</th>
                                <th className="p-3 text-sm font-semibold text-theme-text-secondary">Document Type</th>
                                <th className="p-3 text-sm font-semibold text-theme-text-secondary">Actions</th>
                            </tr>
                        </thead>
                        <tbody>
                            {pendingVerifications.map(profile => (
                                <tr key={profile.id} className="border-b border-theme-border hover:bg-theme-surface/50 text-sm text-theme-text-primary">
                                    <td className="p-3 flex items-center gap-3">
                                        <img src={profile.photo} alt={profile.name} className="h-8 w-8 rounded-full object-cover" />
                                        {profile.name}
                                    </td>
                                    <td className="p-3">
                                        <a 
                                            href={profile.documentForVerification?.url} 
                                            target="_blank" 
                                            rel="noopener noreferrer" 
                                            className="text-blue-400 hover:underline"
                                        >
                                            View {profile.documentForVerification?.type}
                                        </a>
                                    </td>
                                    <td className="p-3 space-x-2">
                                        <button 
                                            onClick={() => handleVerification(profile.id, 'approved')} 
                                            className="text-xs bg-green-500/80 hover:bg-green-500 text-white font-semibold py-1 px-3 rounded-full"
                                        >
                                            Approve
                                        </button>
                                        <button 
                                            onClick={() => handleVerification(profile.id, 'rejected')} 
                                            className="text-xs bg-red-600/80 hover:bg-red-600 text-white font-semibold py-1 px-3 rounded-full"
                                        >
                                            Reject
                                        </button>
                                    </td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
                </div>
            )}
        </div>
    );
};

export default VerificationsManagement;